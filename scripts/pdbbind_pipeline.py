...
def parse_params(config_file=None):
	try:
		if not config_file:
			parser = argparse.ArgumentParser(description="Parsing of config file name",
...
def init_model(params, data, dataset):
	model = Model(hidden_channels=params.MODELINFO.hidden_channels, data=data,
...
def run_pipeline(params, data, model, output_f, tensorboard):
	...
	batches = GraphBatchIterator(data, edge_type=params.MODELINFO.link_prediction_mode, batch_size=batch_size, use_dtis=params.MODELINFO.use_dtis)
	...
	for epoch in range(params.RUNPARAMS.epochs):
		...
		rbo_value = 1 - model.test_rbo(data, set='test') # Test RBO give RBO loss, here we want to highlight RBO itself (1 being perfect)
		...
		if val_loss < val_best_iter[2]:
			val_best_iter = (epoch+1, val_pred, val_loss, rmse_value, rbo_value, pearson_coeff, r2_score, copy.deepcopy(model))
		...
		print(f'Epoch: {epoch+1:03d}, Train RMSE: {train_rmse:.4f}, Test RMSE: {rmse_value:.4f}, RBO: {rbo_value:.4f}, Pearson Coeff (Rp): {pearson_coeff:.4f}, R2: {r2_score:.4f}', file=output_f)
	...
	print(f'Mean absolute difference between targets and predictions at the end: ', (targets-preds).abs().mean().item(), file=output_f)

	print(f'Performances at best: Epoch {best_iter[0]:d} - Test RMSE = {best_iter[2]:.4f}, RBO = {best_iter[3]:.4f}, Pearson Coeff (Rp) = {best_iter[4]:.4f}, R² = {best_iter[5]:.4f}', file=output_f)
	print(f'Performances at best val: Epoch {val_best_iter[0]:d} (Val RMSE = {val_best_iter[2]:.4f}) - Test RMSE = {val_best_iter[3]:.4f}, RBO = {val_best_iter[4]:.4f}, Pearson Coeff (Rp) = {val_best_iter[5]:.4f}, R² = {val_best_iter[6]:.4f}', file=output_f)
...
def plot_results(params, model, dataset, data, best_iter, all_losses, output_f):
	...
	if output_f != sys.stdout:
		pd.DataFrame({'Predictions': preds.cpu(), 'Ground Truths': targets.cpu(), 'Compound': compound_nodes, 'Target': protein_nodes}).to_csv(output_f.name.replace('.log', '.csv'))
...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(description="Parsing of PDB Bind dataset",
											formatter_class=argparse.ArgumentDefaultsHelpFormatter)
	...
	parser.add_argument("--split_type", type=str, help="Type of split ('core' for the 2016 CASF Core Split or 'temporal' for the 2019 Hold-out Split)", default='core')
	...
	params.RUNINFO = params.RUNINFO._replace(run_name = params.RUNINFO.run_name + "_{}_{}".format(split_type, pdbbind_set))
