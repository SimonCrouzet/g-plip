    ...
    try:
        ...
        for seq_record in SeqIO.parse(open(fasta, "r+"), "fasta"):
            if protein_name not in seq_record.id:
                logging.warning(
                    "Ignoring FASTA sequence: Protein name {} not found in fasta file {}".format(protein_name, fasta)
                )
                continue
            else:
                if valid:
                ...
                for i in range(len(seq) - k):
                    ...
                    if khmer not in khmer_dict:
                        logging.error(
                            "Protein features: khmer {} found in protein {}".format(khmer, os.path.split(fasta)[-1])
                        )
                    else:
                        khmer_dict[khmer] += 1
        if not valid:
...
def get_protein_gene_expr(target_ensembl, gene_expr_df, tissue="lung"):
    ...
    if tissue is not None and tissue != "all":
        gene_expressions = gene_expr_df[
            (gene_expr_df.Tissue.str.lower().str.contains(tissue.lower())) & (gene_expr_df.Gene.isin(target_ensembl))
        ]
    else:
...
def generate_esm_encodings(protein_name_list, to_list=True):
    return {
        protein_name: get_esm_encoding(protein_name, to_list)
        for protein_name in tqdm(protein_name_list, desc="Generating ESM2 encodings...")
    }
