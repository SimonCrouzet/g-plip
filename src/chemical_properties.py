from rdkit import Chem, ML, DataStructs
from rdkit.Chem.MolStandardize import rdMolStandardize
from rdkit.Chem.Descriptors import ExactMolWt, TPSA
from rdkit.Chem.rdMolDescriptors import CalcNumRotatableBonds, CalcNumLipinskiHBA, CalcNumLipinskiHBD, CalcNumAromaticRings, CalcNumAliphaticRings
	...
	for comp in compounds:
		...
		if len(comp_smiles) == 0: # No matching SMILES for the compound
			...
			for isotope in comp_smiles:
				parent_isotopes.append(Chem.MolToSmiles(Chem.MolStandardize.rdMolStandardize.IsotopeParent(Chem.MolFromSmiles(isotope))))
			
			if len(np.unique(parent_isotopes)) == 1: # One single parent isotope
				pace_res_df[pace_res_df.CORE == comp].SMILES_standard.replace({isotope: parent_isotopes[0] for isotope in comp_smiles})
				...
				excluded_compounds.append(comp)
				logging.warning('Compound {} has several SMILES representation for several parent isotopes!'.format(comp))
		else:
			if check_compounds_features(comp_smiles[0]):
				updated_comp2smiles[comp] = comp_smiles[0]
			else:
				excluded_compounds.append(comp)
	logging.warning('Dataset: The following compounds were excluded from the pipeline as lacking or confusing SMILES representation from the PACE library:\n{}\n'.format(excluded_compounds))
...
# def merge_similar_smiles(compounds, compound2smiles):
# 	unique_compound2smiles = {}
# 	unique_compounds = []
# 	for comp, comp_smiles in compound2smiles.items():
# 		existing = None
# 		for registered_comp in unique_compounds:
# 			if compare_compounds(comp_smiles, compound2smiles[registered_comp]):
# 				existing = registered_comp
# 				break
# 		if not existing:
# 			unique_compound2smiles[comp] = comp_smiles
# 			unique_compounds.append(comp)
# 		else:
# 			unique_compound2smiles[comp] = existing
# 	return unique_compounds, unique_compound2smiles
...
def get_compound_features(compound_smiles, to_list=False, fingerprint='ECFP', fingerprint_size=124, lipinski_features=True, other_features=True):
	try:
		compound_features = {}
		...
		if lipinski_features:
			...
			compound_features['octanol_water_partition_coef'] = MolLogP(compound_molecule) # Used as a measure of lipophilicity
		...
		if other_features:
			...
			compound_features['percentage_aromaticity'] = sum([compound_molecule.GetAtomWithIdx(i).GetIsAromatic() for i in range(compound_molecule.GetNumAtoms())]) / compound_molecule.GetNumAtoms()

		if fingerprint == 'ECFP':
				# Encoded in 248 bits as adviced by Leo to not overflow the neurons
				fp = GetMorganFingerprintAsBitVect(compound_molecule,2, nBits=fingerprint_size) # Radius 2 so roughly equals to ECFP4
				temp_array = np.zeros((0, ), dtype=np.int8)
		elif fingerprint == 'FCFP':
				fp = GetMorganFingerprintAsBitVect(compound_molecule,2, useFeatures=True, nBits=fingerprint_size) # Radius 2 so roughly equals to FCFP4
				temp_array = np.zeros((0, ), dtype=np.int8)
		...
		print('Chemical features: unable to compute for compound {} due to the following error: {}'.format(compound_smiles, e))
		logging.warning('Compound with uncorrect SMILES: {}. It leads to the following error: {}'.format(compound_smiles, e))
		return None
...
def get_nb_compound_features(fingerprint='ECFP', fingerprint_size=124, lipinski_features=True, other_features=True):
	...
	return len(get_compound_features('C', to_list=True, fingerprint=fingerprint, fingerprint_size=fingerprint_size, lipinski_features=lipinski_features, other_features=other_features))
