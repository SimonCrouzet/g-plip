...
from src.biological_properties import generate_esm_encodings, get_nb_protein_features, get_protein_features, get_union_proteins
from src.chemical_properties import *
from src.external import GeneID_to_UniProtID, UniProtID_to_GeneID, download_uniprot_fasta, exclude_nonhuman_proteins, get_sublocation, get_sublocation_features_length, retrieve_protein_classes
...
def random_edge_split_heterogeneous(data, edge_type, val_perc, test_perc):
	...
	data[edge_type].val_idx = valid_edges[perm[int(train_ratio * valid_edges.shape[0]):int(val_ratio * valid_edges.shape[0])]]
...
def stratified_random_edge_split_heterogeneous(data, edge_type, val_perc, test_perc, exclude_singular=True):
	...
	for node_idx in range(nb_nodes):
		...
		val_idx.extend(valid_edges[perm[int(train_ratio * valid_edges.shape[0]):int(val_ratio * valid_edges.shape[0])]])
...
def stratified_random_train_set_split(data, edge_type, val_perc, test_perc, exclude_singular=False):
	...
	train_idx = []
	val_idx = []
	valid_edges = torch.LongTensor([idx for idx, val in enumerate(data[edge_type].y) if idx not in data[edge_type].test_idx])
...
def scaffold_edge_split_heterogeneous(dataset, data, edge_type, val_perc, test_perc, exclude_singular=True):
	...
	for comp_idx in range(nb_compounds):
		...
		elif comp2scaffold[dataset.compounds[comp_idx]]  in perm_scaffold_idx[int(train_ratio * nb_scaffolds):int(val_ratio * nb_scaffolds)]:
...
class GraphBatchIterator:
	def __init__(self, data, edge_type, batch_size, use_dtis):
		...
		rev_edge_type = (edge_type[1], edge_type[0]) # We need to use edges in both directions in order to correctly pull information

		self.batches = []
		for b_idx in range(0, len(complete_train_idx), batch_size):
			batch = data.cpu().detach().clone() # Create copy of class data
			batch[edge_type].train_idx = complete_train_idx[b_idx:b_idx+batch_size] # Keep only edges belonging to the current batch as train indices
			if use_dtis:
				# Add other train idx (the ones left out from the batch) as real edges on the graph
				known_idx = torch.cat((complete_train_idx[0:b_idx],complete_train_idx[b_idx+batch_size:len(complete_train_idx)]),dim=0)
				...
				batch[rev_edge_type].edge_index = batch[edge_type].y_edge_index[:,known_idx].flip(dims=[0]).detach().clone()
...
def get_missing_links(data, edge_type):
	...
	for from_idx in range(data[edge_from].num_nodes):
		for to_idx in range(data[edge_to].num_nodes):
			...
			existing = torch.where((data[edge_type].y_edge_index[0, :] == from_idx) & (data[edge_type].y_edge_index[1, :] == to_idx))[0]
...
class PDADataset(Dataset):
	...
	def processed_file_names(self):
		...
		if self.settings.COMPOUND_FEATURES.fingerprints:
			base = base + '_Fingerprints{}-{}'.format(self.settings.COMPOUND_FEATURES.fingerprints_type, self.settings.COMPOUND_FEATURES.fingerprints_size)
		...
		if self.settings.COMPOUND_FEATURES.compound_similarities is not None:
			comp_sim_value = str(self.get_compound_similarity_threshold()) if self.get_compound_similarity_threshold() != 0.0 else ''
		...
		if self.settings.PROTEIN_FEATURES.gene_expression:
			base = base + '_GeneExpr'
			if self.settings.PROTEIN_FEATURES.gene_expression_tissue is None or self.settings.PROTEIN_FEATURES.gene_expression_tissue.lower() == 'all':
				base = base + 'All'
			else:
	...
	def read_data(self):
		# Read dataset files
		pace_annot_df = pd.read_csv(os.path.join(self.data_folder, self.settings.INPUT_FILES.pace_annot), delimiter='\t')
		pace_res_df = pd.read_csv(os.path.join(self.data_folder, self.settings.INPUT_FILES.pace_library), delimiter='\t', encoding='windows-1252')
		bds_df = pd.read_csv(os.path.join(self.data_folder, self.settings.INPUT_FILES.bds_screenings), delimiter='\t', low_memory=False)
		chembl_df = pd.read_csv(os.path.join(self.data_folder, self.settings.INPUT_FILES.chembl_data), delimiter='\t', encoding='windows-1252')
		ppi_df = pd.read_csv(os.path.join(self.data_folder, self.settings.INPUT_FILES.ppi_database), header=None, names=['Prot1', 'Prot2'])
		gene_expr_df = pd.read_csv(os.path.join(self.data_folder, self.settings.INPUT_FILES.gene_expr_database), delimiter='\t')

		# Load mapping file
		mapping_file = pd.read_csv(os.path.join(self.data_folder, self.settings.INPUT_FILES.mapping_file), delimiter='\t')

		# Exclude BDS with unclear target
		logging.warning('PDA Dataset: Excluding {} BDS results with unclear target GeneID...'.format(bds_df.TARGET_GENEID.isna().sum()))
		...
		self.targets, excluded_targets = self.check_ensembl_id(self.targets, mapping_file) # Check if every target have a correct EnsemblGeneID, exclude otherwise
		self.targets, excluded_targets = self.check_proteins_validity(self.targets, mapping_file, gene_expr_df) # Check if every target is successfully compiled, exclude otherwise
		...
		if self.settings.INPUT_FILES.restrict_to_pace_compounds:
			self.compounds, self.comp2smiles = check_compounds_smiles_from_PACE(pace_res_df, self.compounds) # Compounds not in the PACE library are excluded here
		else:
	...
	def check_ensembl_id(self, targets, mapping_file):
		...
		logging.warning('Dataset: The following targets were excluded from the pipeline as lacking an EnsemblID:\n{}\n'.format(excluded_targets))
	...
	def check_proteins_validity(self, targets, mapping_file, gene_expr_df):
		...
		for target in tqdm(targets, desc='Checking correctness of targets...'):
			target_ensembl = mapping_file[mapping_file.Accession == target].EnsemblGeneID.unique().tolist()
			try:
				_ = get_protein_features(target, target_ensembl, gene_expr_df, target_esm_encodings, self.data_folder,
				...
				excluded_targets.append(target)
		logging.warning('Dataset: The following targets were excluded from the pipeline as the program was unable to compute protein features:\n{}\n'.format(excluded_targets))
		return [t for t in targets if t not in excluded_targets], excluded_targets
	...
	def build_graph(self):
		...
		print('Initiating a graph containing {} compounds and {} targets...'.format(len(self.compounds), len(self.targets)))

		# Computation of protein features
		# We advice k_khmer to be 3: catalytic site and its neighbours
		target_features = np.empty((len(self.targets), get_nb_protein_features(khmer=self.settings.PROTEIN_FEATURES.khmer,
																				 k_khmer=self.settings.PROTEIN_FEATURES.khmer_size,
																				 sublocation=self.settings.PROTEIN_FEATURES.subcellular_location,
																				 gene_expr=self.settings.PROTEIN_FEATURES.gene_expression,
																				 esm_encoding=self.settings.PROTEIN_FEATURES.esm_encoding)), dtype=np.float32)
		...
		for idx, target in tqdm(enumerate(self.targets), file=sys.stdout, desc='Calculate protein features...'):
			target_ensembl = mapping_file[mapping_file.Accession == target].EnsemblGeneID.unique().tolist()
			target_features[idx, :] = get_protein_features(target, target_ensembl, gene_expr_df, target_esm_encodings, self.data_folder,
																				khmer=self.settings.PROTEIN_FEATURES.khmer,
																				k_khmer=self.settings.PROTEIN_FEATURES.khmer_size,
																				sublocation=self.settings.PROTEIN_FEATURES.subcellular_location,
																				gene_expr=self.settings.PROTEIN_FEATURES.gene_expression,
		...
		fingerprint = self.settings.COMPOUND_FEATURES.fingerprints_type if self.settings.COMPOUND_FEATURES.fingerprints else None
		compound_features = np.empty((len(self.compounds), get_nb_compound_features(fingerprint=fingerprint,
																...
																other_features=self.settings.COMPOUND_FEATURES.extended_chemical_features)), dtype=np.float32)
		for idx, comp in tqdm(enumerate(self.compounds), file=sys.stdout, desc='Calculate compound features...'):
			...
			compound_features[idx, :] = get_compound_features(compound_smiles, to_list=True,
		...
		# Get pAct info
		for target_gene_id, compound_name, pAct in tqdm(dataframe_raw_vectorization_with_numpy(bds_df, keys=['TARGET_GENEID', 'CORE', 'pAct']), file=sys.stdout, desc='Edges pAct'):
			...
			for uniprot_id in target_uniprot_ids:
				if compound_name in self.compounds and uniprot_id in self.targets:
					...
					logging.warning('PDADataset: pAct recorded on unknown drug or target (compound {} - target {})'.format(compound_name, uniprot_id))
		...
		if self.settings.COMPOUND_FEATURES.compound_similarities is not None: # If none, compounds are not linked
			...
			for comp1, comp2 in it.combinations(self.compounds, r=2):
				if self.settings.COMPOUND_FEATURES.compound_similarities: # If True, compounds are linked by compound similarities
					...
					edges_drugs_from_to.append([comp1, comp2])
					edges_drugs_features.append([1.])

		# Format data into numpy arrays
		target_features = np.array(target_features)
		compound_features = np.array(compound_features)
		edges_inter_from_to = np.array([[self.compounds.index(edge_from) for edge_from, _ in edges_inter_from_to], [self.targets.index(edge_to) for _, edge_to in edges_inter_from_to]])
		if self.settings.PROTEIN_FEATURES.protein_protein_interactions:
			edges_ppi_from_to = np.array([[self.targets.index(edge_from) for edge_from, _ in edges_ppi_from_to], [self.targets.index(edge_to) for _, edge_to in edges_ppi_from_to]])
			edges_ppi_features = np.array(edges_ppi_features)
		
		if self.settings.COMPOUND_FEATURES.compound_similarities is not None:
    		# Draw edges unless 'None' compound similarities
			edges_drugs_from_to = np.array([[self.compounds.index(edge_from) for edge_from, _ in edges_drugs_from_to], [self.compounds.index(edge_to) for _, edge_to in edges_drugs_from_to]])
		...
		data['compound', 'interaction', 'target'].edge_index = torch.ones((2,0)).long() # Create empty edge index - for compatibility
		data['compound', 'interaction', 'target'].edge_attr = torch.ones((0,1)).float() # Create empty edge attr - for compatibility
	...
	def add_target_nodes(self, node_names):
		...
		temp_node_names, excluded_targets = self.check_ensembl_id(valid_node_names, mapping_file) # Check if every target have a correct EnsemblGeneID, exclude otherwise
		if len(temp_node_names) == 0:
			logging.error('PDA Dataset: Target {} not found in mapping file'.format(excluded_targets))
			return None
		valid_node_names, excluded_targets = self.check_proteins_validity(temp_node_names, mapping_file, gene_expr_df) # Check if every target is successfully compiled, exclude otherwise
		if len(valid_node_names) == 0:
		...
		target_features = np.empty((len(valid_node_names), get_nb_protein_features(khmer=self.settings.PROTEIN_FEATURES.khmer,
																				k_khmer=self.settings.PROTEIN_FEATURES.khmer_size,
																				sublocation=self.settings.PROTEIN_FEATURES.subcellular_location,
																				gene_expr=self.settings.PROTEIN_FEATURES.gene_expression,
																				esm_encoding=self.settings.PROTEIN_FEATURES.esm_encoding)), dtype=np.float32)
		...
		if self.settings.PROTEIN_FEATURES.protein_protein_interactions:
			...
			for prot1_geneid, prot2_geneid in tqdm(dataframe_raw_vectorization_with_numpy(ppi_df, keys=['Prot1', 'Prot2']), file=sys.stdout, desc='Updating protein-protein interactions...'):
				# We are using a PPI network referencing gene IDs -> Translate them to UniProt IDs (there can be several proteins!)
				prot1_ids = mapping_file[mapping_file.GeneID == prot1_geneid].Accession.to_list()
				prot2_ids = mapping_file[mapping_file.GeneID == prot2_geneid].Accession.to_list()
				for prot1 in prot1_ids:
					for prot2 in prot2_ids:
						if prot1 in valid_node_names or prot2 in valid_node_names: # Concern new proteins
							if prot1 in self.targets and prot2 in self.targets: # Link between two proteins in our graph
								if [prot1, prot2] not in edges_ppi_from_to:
									edges_ppi_from_to.append([prot1, prot2])
									edges_ppi_features.append([1]) #TODO: Improve by providing Confidence Score here
								else:
									logging.warning('PDA Dataset: Trying to duplicate edge ({}, {})'.format(prot1, prot2))
			self.data['target', 'interaction', 'target'].edge_index = torch.LongTensor([self.data['target', 'interaction', 'target'].edge_index[0].tolist() + [self.targets.index(edge_from) for edge_from, _ in edges_ppi_from_to],
									   													self.data['target', 'interaction', 'target'].edge_index[1].tolist() + [self.targets.index(edge_to) for _, edge_to in edges_ppi_from_to]])
			self.data['target', 'interaction', 'target'].edge_attr = torch.cat((self.data['target', 'interaction', 'target'].edge_attr, torch.Tensor(edges_ppi_features)), dim=0)
	...
	def add_compound_nodes(self, node_names, smiles_dict = None):
		...
		# Computation of chemical features
		fingerprint = self.settings.COMPOUND_FEATURES.fingerprints_type if self.settings.COMPOUND_FEATURES.fingerprints else None
		compound_features = np.empty((len(valid_node_names), get_nb_compound_features(fingerprint=fingerprint,
																fingerprint_size=self.settings.COMPOUND_FEATURES.fingerprints_size,
																lipinski_features=self.settings.COMPOUND_FEATURES.lipinski_features,
																other_features=self.settings.COMPOUND_FEATURES.extended_chemical_features)), dtype=np.float32)
		for idx, comp in tqdm(enumerate(valid_node_names), file=sys.stdout, desc='Calculate compound features...'):
			...
			compound_features[idx, :] = get_compound_features(comp, to_list=True,
																fingerprint=fingerprint,
																fingerprint_size=self.settings.COMPOUND_FEATURES.fingerprints_size,
																lipinski_features=self.settings.COMPOUND_FEATURES.lipinski_features,
																other_features=self.settings.COMPOUND_FEATURES.extended_chemical_features)
		...
		if self.settings.COMPOUND_FEATURES.compound_similarities is not None: # If none, compounds are not linked
    		...
    		# Draw edges unless 'None' compound similarities
			self.data['compound', 'similarity', 'compound'].edge_index = torch.LongTensor([self.data['compound', 'similarity', 'compound'].edge_index[0].tolist() + [self.data['compound'].smiles.index(edge_from) for edge_from, _ in edges_drugs_from_to],
									      													self.data['compound', 'similarity', 'compound'].edge_index[1].tolist() + [self.data['compound'].smiles.index(edge_to) for _, edge_to in edges_drugs_from_to]])
			self.data['compound', 'similarity', 'compound'].edge_attr = torch.cat((self.data['compound', 'similarity', 'compound'].edge_attr, torch.Tensor(edges_drugs_features)), dim=0)

		return valid_node_names

	def add_labeled_edges(self, compound_node_names, target_node_names, edge_labels):
		assert len(compound_node_names) == len(target_node_names) == len(edge_labels), 'PDA Dataset: Invalid number of edges'
		...
		if len(valid_edges) != 0:
			self.data['compound', 'target'].y_edge_index = torch.cat((self.data['compound', 'target'].y_edge_index, torch.LongTensor(valid_edges).T), dim=1)
			self.data['compound', 'target'].y = torch.cat((self.data['compound', 'target'].y, torch.Tensor(valid_labels)), dim=0)
			self.data['target', 'compound'].y_edge_index = torch.cat((self.data['target', 'compound'].y_edge_index, torch.LongTensor([edge[::-1] for edge in valid_edges]).T), dim=1)
			self.data['target', 'compound'].y = torch.cat((self.data['target', 'compound'].y, torch.Tensor(valid_labels)), dim=0)

		return valid_edges


	def get_compound_similarity_threshold(self):
		if self.settings.COMPOUND_FEATURES.compound_similarities_threshold >= 0 and self.settings.COMPOUND_FEATURES.compound_similarities_threshold < 1:
			return self.settings.COMPOUND_FEATURES.compound_similarities_threshold
		else:
			logging.warning('PDA Dataset: Compound similarity threshold is expected to be between 0.0 and 1.0 (set at 0.0 by default)')
			return 0.0
	...
	def transform_graph(self, data):
		...
		data['target', 'compound'].y_edge_index = torch.LongTensor([data['compound', 'target'].y_edge_index[1,:].tolist() , data['compound', 'target'].y_edge_index[0,:].tolist()])
	...
	def randomize_edges(self, seed=123, random_pAct=True):
		...
		if random_pAct:
			y_perm = Uniform(low=3., high=11.).sample((nb_edges, 1))
		else:
			y_perm = data['compound', 'target'].y[torch.randperm(nb_edges)]

		# Randomly choose nodes in relation
		data['compound', 'target'].y_edge_index = torch.LongTensor([torch.randint(low=0,high=len(self.compounds), size=(nb_edges,)).tolist(), torch.randint(low=0,high=len(self.targets), size=(nb_edges,)).tolist()])
	...
	def get(self):
		...
		if os.path.isfile(os.path.join(self.data_folder, self.processed_file_names)):
			print('PDA Dataset: Loading data from {} ...'.format(os.path.join(self.data_folder, self.processed_file_names)))
	...
	def get_metadata(self, use_dtis):
		...
		if use_dtis: # Here we'll use the target-compounds in the train set
			return (['target', 'compound'], [('target', 'interaction', 'target'), ('compound', 'similarity', 'compound'), ('compound', 'interaction', 'target'), ('target', 'rev_interaction', 'compound')])
		else:
			return (['target', 'compound'], [('target', 'interaction', 'target'), ('compound', 'similarity', 'compound')])
...
def combine_datasets(original_dataset, additional_dataset, val_size, save_path='combined_dataset.pt'):
	...
	comp_from = [additional_dataset.data['compound'].smiles[i] for i in additional_dataset.data['compound','target'].y_edge_index[0]]
	target_to = [additional_dataset.data['target'].names[i] for i in additional_dataset.data['compound','target'].y_edge_index[1]]
	edges_added = combined_dataset.add_labeled_edges(compound_node_names=comp_from, target_node_names=target_to, edge_labels=additional_dataset.data['compound','target'].y.flatten().tolist())
	...
	combined_dataset.data['target','compound'].test_idx = torch.LongTensor([i for i in range(total_nb_edges)])[-nb_edges_added:]
	combined_dataset.data['compound','target'].train_idx = combined_dataset.data['target','compound'].train_idx.clone()
...
def export_to_dataframe(data, dataset_name):
	...
	df = pd.DataFrame(index=range(data['target','compound'].y.shape[0]), columns=['target','compound','set','pAct']+['fea_target_{}'.format(i) for i in range(nb_fea_targets)]+['fea_compound_{}'.format(i) for i in range(nb_fea_compounds)])
	for idx, ((target_idx, compound_idx), pact) in enumerate(zip(data['target','compound'].y_edge_index.T.tolist(), data['target','compound'].y.flatten().tolist())):
		...
		df.loc[idx, ['fea_compound_{}'.format(i) for i in range(nb_fea_compounds)]] = data['compound'].x[compound_idx].tolist()
...
def select_test_set(dataset):
	...
	mask_compcomp = np.isin(dataset.data['compound','compound'].edge_index[0,:], unique_compounds) & np.isin(dataset.data['compound','compound'].edge_index[1,:], unique_compounds)
	mask_tartar = np.isin(dataset.data['target','target'].edge_index[0,:], unique_targets) & np.isin(dataset.data['target','target'].edge_index[1,:], unique_targets)

	reduced_dataset = copy.deepcopy(dataset)
	reduced_dataset.data['compound'].smiles = np.array(reduced_dataset.data['compound'].smiles)[unique_compounds].tolist()
	reduced_dataset.data['compound'].names = np.array(reduced_dataset.data['compound'].names)[unique_compounds].tolist()
	...
	reduced_dataset.data['compound','compound'].edge_index = torch.LongTensor([[reduced_dataset.data['compound'].smiles.index(original_compounds[i]) for i in reduced_dataset.data['compound','compound'].edge_index[0, mask_compcomp]],
																			[reduced_dataset.data['compound'].smiles.index(original_compounds[i]) for i in reduced_dataset.data['compound','compound'].edge_index[1, mask_compcomp]]])
	reduced_dataset.data['compound','compound'].edge_attr = reduced_dataset.data['compound','compound'].edge_attr[mask_compcomp]
	reduced_dataset.data['target','target'].edge_index = torch.LongTensor([[reduced_dataset.data['target'].names.index(original_targets[i]) for i in reduced_dataset.data['target','target'].edge_index[0, mask_tartar]],
									      									[reduced_dataset.data['target'].names.index(original_targets[i]) for i in reduced_dataset.data['target','target'].edge_index[1, mask_tartar]]])
	reduced_dataset.data['target','target'].edge_attr = reduced_dataset.data['target','target'].edge_attr[mask_tartar]
	reduced_dataset.data['target','compound'].y_edge_index = torch.LongTensor([[reduced_dataset.data['target'].names.index(original_targets[i]) for i in test_edge_indices[0,:]],
	...
	reduced_dataset.data['target','compound'].test_idx = torch.LongTensor([i for i in range(test_edges_labels.shape[0])])
	reduced_dataset.data['compound','target'].y_edge_index = torch.LongTensor([[reduced_dataset.data['compound'].smiles.index(original_compounds[i]) for i in test_edge_indices[1,:]],
