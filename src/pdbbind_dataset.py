...
from src.chemical_properties import compare_compounds
from src.external import exclude_nonhuman_proteins, download_uniprot_fasta
...
class PDBBindDataset(PDADataset):
	...
	def read_data(self):
		...
		try:
			...
			pdbbind_general_df = pd.read_csv(os.path.join(pdbbind_data_folder, 'index', 'INDEX_general_PL_data.2020'), delimiter=r"\s{2,}", engine='python', skiprows=6, header=None, on_bad_lines='skip')
			pdbbind_general_df = pdbbind_general_df.drop(columns=[4,5]).rename(columns={0: 'PDB code', 1:'resolution', 2:'release year', 3:'-logKd/Ki'})
			pdbbind_name_df = pd.read_csv(os.path.join(pdbbind_data_folder, 'index', 'INDEX_general_PL_name.2020'), delimiter=r"\s{2,}", engine='python', skiprows=6, header=None, on_bad_lines='skip')
			pdbbind_name_df = pdbbind_name_df.drop(columns=[1,3]).rename(columns={0: 'PDB code', 2:'UniProt ID'})
		except:
			raise ValueError('PDB Bind data not found in {} - please fix your folder containing PDB data accordingly'.format(pdbbind_data_folder))
		...
		for pdb_code in pdbbind_all_index['PDB code']:
			...
			try:
				...
				mol = Chem.MolFromMol2File(os.path.join(pdbbind_data_folder, pdb_code, "{}_ligand.mol2".format(pdb_code)))
				if mol is None:
					# If MOL2 file didn't work, try with SDF file
					mol = Chem.SDMolSupplier(os.path.join(pdbbind_data_folder, pdb_code, "{}_ligand.sdf".format(pdb_code)))[0]
		...
		self.targets = exclude_nonhuman_proteins(self.targets, json_path=os.path.join(self.data_folder, 'pdbbind_human_proteins.json'))
		...
		if os.path.isfile(smiles_eq_path):
			with open(smiles_eq_path) as json_file:
				...
				smiles_to_equivalent, invalid_smiles = smiles_to_equivalent_save['smiles_dict'], smiles_to_equivalent_save['invalid_smiles']
		else:
		...
		self.compounds = np.unique([v for k,v in smiles_to_equivalent.items() if k in pdbbind_all_index['SMILES'].tolist()]).tolist()
	...
	def build_graph(self):
		...
		for idx, compound_smiles in enumerate(tqdm(self.compounds, file=sys.stdout, desc='Compound features')):
			compound_features[idx, :] = get_compound_features(smiles_to_equivalent[compound_smiles], to_list=True,
		...
		# Get pAct info
		for target_uniprot_id, compound_smiles_name, pAct, year in tqdm(dataframe_raw_vectorization_with_numpy(pdbbind_all_index, keys=['UniProt ID', 'SMILES', '-logKd/Ki', 'release year']), file=sys.stdout, desc='Edges pAct'):
			if compound_smiles_name in self.compounds and target_uniprot_id in self.targets:
				...
				logging.warning('PDADataset: pAct recorded on unknown drug or target (compound {} - target {})'.format(compound_smiles_name, target_uniprot_id))
			...
			for compound_smiles_name in self.compounds:
				...
				if edge_inter_dict_holdout[uniprot_id][compound_smiles_name]: # list is not empty
					...
					if self.settings.INPUT_FILES.average_screenings:
						...
						# Pick randomly one of the pAct and fill in edges
						edges_inter_y.append([random.choice(edge_inter_dict_holdout[uniprot_id][compound_smiles_name])])
		...
		for prot1_geneid, prot2_geneid in tqdm(dataframe_raw_vectorization_with_numpy(ppi_df, keys=['Prot1', 'Prot2']), file=sys.stdout, desc='PPI edges'):
		...
		if self.settings.COMPOUND_FEATURES.compound_similarities is not None:
			...
			for comp1, comp2 in tqdm(it.combinations(self.compounds, r=2), file=sys.stdout, desc='Compound similarity edges'):
				if self.settings.COMPOUND_FEATURES.compound_similarities:
					# Use compound similarities on the edges
					# edges_drugs_features.append(compounds_similarity(get_compound_smiles(comp1, pace_res_df), get_compound_smiles(comp2, pace_res_df)))
	...
	def get(self):
		if os.path.isfile(os.path.join(self.data_folder, self.processed_file_names)):
			print('PDBBind Dataset: Loading data from {} ...'.format(os.path.join(self.data_folder, self.processed_file_names)))
	...
	def export_data(self, data, only_test=False, file_name=None):
		...
		if only_test:
			...
			all_sets = ['train' if i in data['compound', 'target'].train_idx else 'val' if i in data['compound', 'target'].val_idx else 'test' for i in range(len(all_pAct))]
			df = pd.DataFrame(data={'compound': all_compounds, 'target': all_targets, 'pAct': all_pAct, 'set': all_sets})
...
def construct_pdbbind_data(params, output_f, pdbbind_set='complete'):
	data_folder = params.RUNINFO.data_dir
	...
	if params.RUNPARAMS.val_size > 0.0:
		data = stratified_random_train_set_split(data, edge_type=params.MODELINFO.link_prediction_mode, val_perc=params.RUNPARAMS.val_size, test_perc=params.RUNPARAMS.test_size)
		data[list(reversed(params.MODELINFO.link_prediction_mode))].train_idx = data[params.MODELINFO.link_prediction_mode].train_idx.clone()
		data[list(reversed(params.MODELINFO.link_prediction_mode))].val_idx = data[params.MODELINFO.link_prediction_mode].val_idx.clone()
		data[list(reversed(params.MODELINFO.link_prediction_mode))].test_idx = data[params.MODELINFO.link_prediction_mode].test_idx.clone()
...
def casf_core_split(dataset, params, casf_subfolder='CASF-2016'):
	...
	target_df = pd.read_csv(os.path.join(casf_folder, 'power_screening', 'TargetInfo.dat'), skiprows=8, sep='\s+')
	...
	for pdbcode, uniprotid in zip(pdb_mapping_df['PDBCode'], pdb_mapping_df['UniProtID']):
		if pdbcode in pdb2uniprot:
			...
			if pdbcode == '4w9h':
				...
				print('PDB code {} is mapped to multiple UniProt IDs: {} and {}'.format(pdbcode, pdb2uniprot[pdbcode], uniprotid))
		else:
	...
	for pdbcode, t1, t2, target_id in zip(ligand_df['#code'], ligand_df['T1'], ligand_df['T2'], ligand_df['group']):
		...
		if os.path.isfile(os.path.join(casf_folder, 'coreset', pdbcode, pdbcode + '_ligand_opt.mol2')):
			ligand_smiles = get_smiles_from_mol2(os.path.join(casf_folder, 'coreset', pdbcode, pdbcode + '_ligand_opt.mol2'))
		elif os.path.isfile(os.path.join(casf_folder, 'coreset', pdbcode, pdbcode + '_ligand.mol2')):
			ligand_smiles = get_smiles_from_mol2(os.path.join(casf_folder, 'coreset', pdbcode, pdbcode + '_ligand.mol2'))
		else:
	...
	ids2target = {i+1:t for i,t in enumerate(target_df['#T'].values)}
	...
	# all_human_proteins = exclude_nonhuman_proteins(list(unique_targets), json_path=os.path.join(self.data_folder, 'pdbbind_human_proteins.json'))
	...
	dataset.add_labeled_edges(compound_node_names=[x[1] for x in target_ligand_pair], target_node_names=[x[0] for x in target_ligand_pair], edge_labels=target_ligand_labels)

	existing_edges = [(t.item(), l.item()) for t,l in dataset.data['target','compound'].y_edge_index.T]
	casf_edges = [(dataset.data['target'].names.index(x[0]), dataset.data['compound'].names.index(x[1])) for x in target_ligand_pair]

	all_train_edges = torch.LongTensor([edge_idx for edge_idx, edge in enumerate(existing_edges) if edge not in casf_edges])
