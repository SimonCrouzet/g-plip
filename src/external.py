...
def request_sublocation_from_uniprot(uniprot_id):
	url = 'https://rest.uniprot.org/uniprotkb/search?query=accession:{}+AND+organism_id:9606&format=tsv&fields=cc_subcellular_location'.format(uniprot_id)
...
def GeneID_to_UniProtID(gene_ids, return_list=False):
	...
	result = subprocess.check_output("curl --form 'from=GeneID' \
							--form 'to=UniProtKB' \
							--form 'ids={}' \
							https://rest.uniprot.org/idmapping/run".format(gene_ids), shell=True)
	job_id = json.loads(result)["jobId"]

	# Give time to UniProt to execute the job
	check_id_mapping_results_ready(job_id)

	url = "https://rest.uniprot.org/idmapping/uniprotkb/results/{}/?fields=accession&format=tsv&size=500".format(job_id)
...
def UniProtID_to_GeneID(uniprot_ids, return_list=False):
	...
	result = subprocess.check_output("curl --form 'from=UniProtKB_AC' \
							--form 'to=GeneID' \
							--form 'ids={}' \
							https://rest.uniprot.org/idmapping/run".format(",".join(uniprot_ids)), shell=True)
	job_id = json.loads(result)["jobId"]
...
def exclude_nonhuman_proteins(proteins, json_path=None):
	...
	if json_path and os.path.isfile(json_path) and 'human_proteins' in json.loads(open(json_path, encoding='utf-8').read()):
		return json.loads(open(json_path, encoding='utf-8').read())['human_proteins']
	else:
		human_proteins = []
		for protein in proteins:
			try:
				url = 'https://rest.uniprot.org/uniprotkb/search?query=accession:{}&format=tsv&fields=organism_id'.format(protein)
				df = pd.read_csv(url, delimiter='\t')
				if len(df) == 0:
					logging.warning('UniProt: UniProtID {} has been redirected - please manually update your database!'.format(protein))
				elif len(df) > 1:
...
def download_uniprot_fasta(proteins, data_folder):
	...
	for uniprot_id in proteins:
		if not os.path.isfile(os.path.join(data_folder, 'fasta', uniprot_id+'.fasta')):
			url = 'https://rest.uniprot.org/uniprotkb/search?query={}+AND+organism_id:9606&format=fasta&compressed=false'.format(uniprot_id)
...
def ProteinName_to_UniProtID(protein_name):
	# Retrieve UniProtID from Protein name - DEPRECATED
	url = "https://rest.uniprot.org/uniprotkb/search?fields=accession%2Cid%2Cprotein_name&format=tsv&query=%28%28protein_name%3A%22{}%22%29%20AND%20%28organism_id%3A9606%29%20AND%20%28reviewed%3Atrue%29%29&size=500".format(protein_name.replace(' ', '%20'))
	df = pd.read_csv(url, delimiter='\t')

	if len(df) == 0:
		logging.warning('UniProt: UniProtID of protein {} has not been found.'.format(protein_name))
		return ''
	elif len(df) > 1:
		df = filter_protein_name(df, protein_name)
		if len(df) == 0:
			logging.warning('UniProt: UniProtID of protein {} has not been found.'.format(protein_name))
			return ''
		elif len(df) > 1:
			logging.warning('UniProt: Protein {} returns several UniProtIDs.'.format(protein_name))
			return ''
	return df['Entry'].iloc[0]

def PDBCode_to_UniProtID(pdbcodes, return_list=False):
	# Retrieve UniProtID from PDB Code
	result = subprocess.check_output("curl --form 'from=PDB' \
							--form 'to=UniProtKB' \
							--form 'ids={}' \
							https://rest.uniprot.org/idmapping/run".format(','.join(pdbcodes)), shell=True)
	job_id = json.loads(result)["jobId"]
...
def retrieve_protein_classes(proteins):
	...
	keyword_categories = ['Biological process', 'Molecular function', 'Disease', 'Cellular component'] # We are interested in 4 different categories only
	category2keywords = {c:uniprot_keyword_df[uniprot_keyword_df.Category == c].Name.unique().tolist() for c in keyword_categories}
	protein_class_df = pd.DataFrame(columns=keyword_categories, index=proteins)
	for protein in proteins:
		url = 'https://rest.uniprot.org/uniprotkb/search?query=accession:{}+AND+organism_id:9606&format=tsv&fields=keyword'.format(protein)
		df = pd.read_csv(url, delimiter='\t')
		keywords_protein_list = df.Keywords.loc[0].split(';')
		protein_class_df.loc[protein] = {c:[k for k in keywords_protein_list if k in category2keywords[c]] for c in keyword_categories}
